...
+!update_targets <-
	  ...
	  if(corner(0)){
	      ...
	      if(corner(1)){
	          !safe_pos(INITIALFLAGX+DISTANCE,0,INITIALFLAGZ+ROUND);
			  ?safe_pos(XSAFE,YSAFE,ZSAFE);
			  .println("YENDO A: ",XSAFE,",0,",ZSAFE);
			  -+corner(2);
	          ...
	          if(corner(2)){
			      ...
			      if(corner(3)){
					!safe_pos(INITIALFLAGX-ROUND,0,INITIALFLAGZ+DISTANCE);
					?safe_pos(XSAFE,YSAFE,ZSAFE);
					.println("YENDO A: ",XSAFE,",0,",ZSAFE);
					!add_task(task(3000,"TASK_GOTO_POSITION3",Me,pos(XSAFE,YSAFE,ZSAFE),""));
					-+corner(4);
...
+!init
   <- 
     ?objective(INITIALFLAGX,INITIALFLAGY,INITIALFLAGZ);
     +pos_initial_flag(INITIALFLAGX,0,INITIALFLAGZ);
     -+tasks([]);
     +round(10); //variable de distancia de vuelta hasta la bandera.
     +distance(5);
     +corner(0). 
